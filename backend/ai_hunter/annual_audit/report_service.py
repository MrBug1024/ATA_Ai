...
def _generation_key(payload: dict[str, Any]) -> str:
    # Release/readiness evaluation timestamps describe when the same facts
    # were observed, not a change in those facts.  Including them would make
    # an unchanged report generation create a new report version.
    volatile_keys = {
        "evaluated_at",
        "generated_at",
        "observed_at",
    ...
    raw = json.dumps(stable(payload), ensure_ascii=False, sort_keys=True, default=_json_default)
...
def generate_annual_report_draft(
    ...
    *,
    ...
    snapshot = {
        ...
        # Keep the identity and period in the frozen fact snapshot so the
        # persisted report retains the complete engagement context.
        "engagement": {
