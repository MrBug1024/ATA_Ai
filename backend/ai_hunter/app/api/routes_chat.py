...
def _build_graph_input(payload: ChatRequest, identity: Identity | None = None) -> dict[str, Any]:
    ...
    # Actor fields are part of the audit trail and idempotency namespace.
    # Never trust the caller-supplied operator fields on a chat payload.
    data["operator_id"] = str(identity_context.get("user_id") or "ai_agent")
    data["operator_name"] = str(
        identity_context.get("username") or data["operator_id"]
    )
