...
AUDIT_ACTION_MARKERS = ("执行", "开始", "启动", "开展", "进行", "完成", "落实", "按计划", "继续")
AUDIT_SCOPE_MARKERS = ("年度审计", "年审", "审计")
...
def _report_exists(state: AuditGraphState) -> bool:
    ...
    return bool(
        ...
        or focus.get("report_exists")
        or focus.get("final_report_ref")
    )


def _is_explicit_audit_execution_request(query: str) -> bool:
    text = str(query or "").strip()
    if not text or any(marker in text for marker in NON_EXECUTION_MARKERS):
        return False
    return bool(
        any(marker in text for marker in AUDIT_SCOPE_MARKERS)
        and any(marker in text for marker in AUDIT_ACTION_MARKERS)
    )


def _is_audit_workflow_request(query: str) -> bool:
    text = str(query or "")
    return bool(
        any(marker in text for marker in AUDIT_SCOPE_MARKERS)
        and any(marker in text for marker in NON_EXECUTION_MARKERS)
    )


def _is_explicit_reaudit_request(query: str) -> bool:
    text = str(query or "")
    has_action = any(marker in text for marker in REAUDIT_KEYWORDS) or "重新生成" in text
    has_correction = bool(
        re.search(r"(?:应为|改为|正确值|更正为|调整为).{0,20}(?:\d|元|万元|%|百分比)", text)
        or any(marker in text for marker in ("根据更正", "按上述更正", "更正后"))
    )
    return has_action and has_correction
...
def _rule_route(state: AuditGraphState) -> RouteDecisionModel | None:
    ...
    if _is_explicit_reaudit_request(query):
    ...
    if (
        query in GREETING_KEYWORDS
        or any(query.startswith(keyword) for keyword in GREETING_KEYWORDS)
        or any(keyword in query for keyword in GENERAL_HELP_KEYWORDS)
    ):
...
def _apply_requirements(decision: RouteDecisionModel, state: AuditGraphState) -> RouteDecisionModel:
    ...
    if spec is not None and spec.access_mode == "write":
        ...
        semantic_audit_write = (
            decision.capability == "audit.full"
            and _is_explicit_audit_execution_request(str(state.get("query") or ""))
        ) or (
            decision.capability == "audit.reaudit"
            and _is_explicit_reaudit_request(str(state.get("query") or ""))
        )
        if decision.source == "model" and explicit != decision.capability and not semantic_audit_write:
            decision.needs_clarification = True
    ...
    try:
        ...
        if has_api_key(llm):
            context = {
                ...
                "conversation_focus": state.get("conversation_focus") or {},
                "memory_context": str(state.get("memory_context") or "")[-6000:],
            }
            messages = [
                SystemMessage(
            ...
            try:
                ...
                decision = _coerce_route_model(response)
            except Exception as structured_exc:
                LOGGER.warning("annual_router_structured_parse_failed error=%s", structured_exc)
                decision = _coerce_route_model(llm.invoke(messages))
...
def _message_content(value: object) -> str:
    ...
    return json.dumps(content, ensure_ascii=False, default=str) if isinstance(content, dict) else str(content or "")


def _coerce_route_model(value: object) -> RouteDecisionModel:
    if isinstance(value, RouteDecisionModel):
        return value
    if isinstance(value, dict):
        parsed = value.get("parsed")
        if parsed is not None:
            return parsed if isinstance(parsed, RouteDecisionModel) else RouteDecisionModel.model_validate(parsed)
        if "capability" in value:
            return RouteDecisionModel.model_validate(value)
        value = value.get("raw") or value
    text = _message_content(value).strip()
    ...
    return RouteDecisionModel.model_validate(json.loads(candidate))
