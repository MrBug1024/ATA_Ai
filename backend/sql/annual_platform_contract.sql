...
-- PostgreSQL audit_engagement.id in the unified annual-audit database.
...
'年审 LangGraph 会话权限入口；case_id 表示 PostgreSQL audit_engagement.id。';
COMMENT ON COLUMN public.thread_metadata.case_id IS
'年审项目兼容 ID，权威项目与成员关系存储于 PostgreSQL。';
